    ...
    with connection.cursor() as cursor:
        ...
        for index in range(1, 1000):
            ...
            for i in range(0, 10000):
                domain_name = uuid.uuid4().__str__()
                name = "openssl \"ssl-death-alert\" 拒绝服务漏洞(cve-2016-8610)【原理扫描】" + str(index)
                value = f"""('112.17.34.91', '0000000000000000000000007011225B', '{domain_name}', -1, null, null,
...
VALUES {insert_values};"""
            ...
            connection.commit()
            logger.info(f"insert with index:{index}, cost:{time.time() - start_time} seconds")


def insert_with_no_key():
    # drop_table_sql = """drop table if exists siem_loophole_log_migration_insert_no_key;"""
    create_table_sql = """CREATE TABLE if not exists `siem_loophole_log_migration_insert_no_key` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '自增id',
  `host` varchar(39) NOT NULL DEFAULT '' COMMENT '漏洞发现地址',
  `hex_host` char(32) NOT NULL DEFAULT '' COMMENT '漏洞发现地址对应十六进制',
  `domain_name` varchar(255) NOT NULL DEFAULT '' COMMENT '域名',
...
def insert_with_compress():
    # drop_table_sql = """drop table if exists siem_loophole_log_migration_insert_compress;"""
    create_table_sql = """CREATE TABLE if not exists `siem_loophole_log_migration_insert_compress` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '自增id',
  `host` varchar(39) NOT NULL DEFAULT '' COMMENT '漏洞发现地址',
  `hex_host` char(32) NOT NULL DEFAULT '' COMMENT '漏洞发现地址对应十六进制',
  `domain_name` varchar(255) NOT NULL DEFAULT '' COMMENT '域名',
  ...
  KEY `idx_scan_id` (`scan_id`),
  KEY `idx_last_scan_time` (`last_scan_time`)
) ENGINE=InnoDB ROW_FORMAT=COMPRESSED DEFAULT CHARSET=utf8mb3 COMMENT='漏洞扫描信息表';"""
    with connection.cursor() as cursor:
        # cursor.execute(drop_table_sql)
        cursor.execute(create_table_sql)
        ...
        '2021-04-24 08:47:38', '2021-04-24 08:47:38', '2022-03-09 16:21:34', '2022-03-09 16:21:34')"""
            ...
            insert_values = str.join(", ", values)
            insert_with_id_sql = f"""INSERT INTO siem_loophole_log_migration_insert_compress (host, hex_host, domain_name, port, asset_name, owner_name, name,
                                         risk_level, siem_risk_level, loophole_id, source_type, source, source_id,
                                         status, scan_id, scan_host, description_id, solution_id,
                                         disposal_recommendations_id, cve_time, cve_id, cnnvd_id, cncve_id, cnvd_id,
                                         bugtraq, cvss, discovery_time, last_scan_time, create_time, update_time)
    ...
    with connection.cursor() as cursor:
        ...
        for index in range(1, 1000):
            ...
            for i in range(0, 10000):
                ...
                domain_name = uuid.uuid4().__str__()
                name = "openssl \"ssl-death-alert\" 拒绝服务漏洞(cve-2016-8610)【原理扫描】" + str(index)
                value = f"""('{id}', '112.17.34.91', '0000000000000000000000007011225B', '{domain_name}', -1, null, null,
            '{name}', 2, 2, 'RSAS777_76960_112.17.34.91', 0, 'rsas', '', 0,
            'RSAS777_76960_112.17', '', 1223, 580, null, '2022-03-25 11:41:21', 'CVE-2016-8610', '', '', '', '', null,
            '2021-04-24 08:47:38', '2021-04-24 08:47:38', '2022-03-09 16:21:34', '2022-03-09 16:21:34')"""
                values.append(value)
