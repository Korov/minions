...
[source,bash]
----
docker rmi korov/minions-spider:1.1
docker build -f ./Dockerfile -t korov/minions-spider:1.1 .
docker run -d -it --restart=unless-stopped --name minions-spider-biquge korov/minions-spider:1.1 pipenv run scrapy crawl biquge
